...
-- RLS Policies for users (Drop and Recreate for re-run safety)
DROP POLICY IF EXISTS "Users can read own profile" ON public.users;
CREATE POLICY "Users can read own profile" 
  ON public.users FOR SELECT 
  USING (auth.uid() = id);

DROP POLICY IF EXISTS "Users can update own whatsapp phone" ON public.users;
...
-- RLS Policies for user_preferences (Drop and Recreate for re-run safety)
...
DROP POLICY IF EXISTS "Games cache is publicly readable" ON public.games_cache;
...
-- RLS Policies for daily_picks (Drop and Recreate for re-run safety)
DROP POLICY IF EXISTS "Users can view own daily picks" ON public.daily_picks;
...
CREATE TABLE IF NOT EXISTS public.treasure_alerts (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
  game_id TEXT NOT NULL,
  game_name TEXT NOT NULL,
...
-- RLS Policies for treasure_alerts (Drop and Recreate for re-run safety)
DROP POLICY IF EXISTS "Users can view own treasure alerts" ON public.treasure_alerts;
CREATE POLICY "Users can view own treasure alerts" 
  ON public.treasure_alerts FOR SELECT 
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own treasure alerts" ON public.treasure_alerts;
...
CREATE TABLE IF NOT EXISTS public.silver_ad_clicks (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
...
-- RLS Policies for silver_ad_clicks (Drop and Recreate for re-run safety)
DROP POLICY IF EXISTS "Users can view own ad clicks" ON public.silver_ad_clicks;
CREATE POLICY "Users can view own ad clicks" 
  ON public.silver_ad_clicks FOR SELECT 
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own ad clicks" ON public.silver_ad_clicks;
CREATE POLICY "Users can insert own ad clicks" 
  ON public.silver_ad_clicks FOR INSERT 
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own ad clicks" ON public.silver_ad_clicks;
...
CREATE TABLE IF NOT EXISTS public.golden_payments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
...
-- RLS Policies for golden_payments (Drop and Recreate for re-run safety)
DROP POLICY IF EXISTS "Users can view own golden payments" ON public.golden_payments;
...
-- Create or update the function that inserts a row into public.users
