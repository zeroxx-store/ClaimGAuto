  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // If a session is automatically created
        if (data.session) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0a0a0a] text-white px-4 relative">
      ...
      <div className="w-full max-w-md bg-[#1f1f1f]/50 backdrop-blur-xl rounded-3xl border border-white/5 shadow-2xl p-8 sm:p-10 relative">
        ...
        <div className="text-center mb-8 flex flex-col items-center">
          ...
          <p className="text-gray-500 text-xs sm:text-sm mt-1">Start tracking epic free and discounted game offers</p>
        </div>

        {errorMsg && (
          <div className="mb-4 bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-3 rounded-xl text-xs sm:text-sm font-semibold animate-in fade-in">
            {errorMsg}
          </div>
        )}

        {infoMsg && (
          <div className="mb-4 bg-[#00d4ff]/10 border border-[#00d4ff]/20 text-[#00d4ff] px-4 py-3 rounded-xl text-xs sm:text-sm font-semibold animate-in fade-in">
            {infoMsg}
          </div>
        )}
        ...
        <form onSubmit={handleSignUp} className="space-y-4">
          ...
          {/* Password field */}
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold uppercase tracking-wider text-gray-400">Choose Password</label>
        ...
        <p className="text-center text-gray-500 text-xs sm:text-sm mt-8 font-medium">
          ...
          <Link href="/login" className="text-[#00d4ff] hover:underline font-bold transition">
            Log In
          </Link>
        </p>
      </div>
